...
PHASE 0 IN PROGRESS — Steps **0.1–0.5 are complete** and verified. 0.1 monorepo & tooling; 0.2 `db` service (pgvector); 0.3 NestJS API + health checks; 0.4 Prisma schema (19 core entities, §5) + initial migration; 0.5 seed data — the 30 methodology blocks (Appendix A/F), carrier presets (Appendix B), and a settings singleton, idempotent and exercised by integration tests. Active item: **step 0.6 (auth & RBAC) — in progress**. Spec adopted (`docs/spec/psa-mvp.md`; ADR `docs/decisions/0001-...`); plan at `docs/plans/phase-0-skeleton-and-infrastructure.md`.
...
- Plan (ordered, each step independently committable):
  ...
  - [~] 0.6 Auth & RBAC: email/password (argon2), JWT cookie, CSRF, rate limit, role guards, data isolation; tests — IN PROGRESS
...
- How to resume: start at Step 0.6 (auth & RBAC). Add an `auth` module (email/password with argon2, session cookie or JWT, CSRF, rate-limit on auth routes), role guards for admin/architect/client, and a data-isolation guard (architect sees only own partnerships; client scoped via `client_access`). Bootstrap an initial admin from env. Reuse `USER_ROLES`/`isUserRole` (`@psa/shared`) and the Prisma `Role` enum. Dev DB + data: `docker compose --env-file deploy/.env -f deploy/docker-compose.yml up -d db`, `pnpm --filter @psa/api db:deploy`, `pnpm --filter @psa/api db:seed`. Authoring migrations: `prisma migrate diff` (avoid `prisma migrate reset` — gated by a dangerous-action guard).
