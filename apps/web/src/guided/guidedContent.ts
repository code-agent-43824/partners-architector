...
export const GUIDED_SCREENS: GuidedScreen[] = [
  ...
  {
    kind: 'partnership',
    title: 'Карточка кейса — шаг 1 из 4',
    lead: 'Кейс ведётся в четыре шага — навигация сверху: 1) о партнёрстве, 2) участники, 3) совместимость, 4) сессии. Здесь, на первом шаге, — название, тип и рабочие заметки дела.',
    how: [
  ...
  },
  {
    ...
    how: [
      ...
      'Когда участников не меньше двух, нажмите «Далее: совместимость».',
    ],
    future: [
  ...
  {
    kind: 'compatibility',
    title: 'Совместимость — шаг 3 из 4',
    lead: 'Если партнёры прошли «Тест естественной совместимости партнёров» (ПЕСП), загрузите сюда результаты. Красные и жёлтые зоны отчёта подсветят соответствующие вопросы в сценарии сессии — повестка перестанет быть одинаковой для всех. Шаг необязательный: без теста сессия идёт как обычно.',
    how: [
      'Нажмите «Импортировать результаты (файл)» и выберите файл отчёта — подойдёт любой формат, файл сохранится.',
      'Если формат пока не распознаётся автоматически, отметьте зоны из отчёта вручную — по конструктам в трёх блоках: различие, паритет, совпадение.',
      'Откройте сценарий сессии: блоки с зонами внимания отмечены цветом, внутри блока видно, какой конструкт его подсветил.',
    ],
    future: [
      'Прохождение теста прямо здесь: партнёрам уйдут персональные ссылки, ответы соберутся сами.',
      'Автоматическое распознавание официального отчёта ПЕСП и попарные матрицы для 3–5 партнёров.',
      'Сравнение профилей между раундами тестирования — к сессиям актуализации.',
    ],
  },
  {
    kind: 'sessions',
    title: 'Сессии — шаг 4 из 4',
    ...
    pathname === '/'
      ...
      : /^\/partnerships\/[^/]+$/.test(pathname)
        ...
        : /^\/partnerships\/[^/]+\/partners$/.test(pathname)
          ? 'partners'
          : /^\/partnerships\/[^/]+\/compatibility$/.test(pathname)
            ? 'compatibility'
